...
/** A version of `ConfigCursor` with a more fluent interface, focused on config navigation instead of error handling.
  ...
  * always returns another cursor instead of a `ConfigReader.Result`. The error handling is left for the last step,
  * where users can opt to cast to a primitive value using one of the `as` methods or by requesting a regular cursor
  * with `cursor`.
  *
  * @param cursor
  *   the regular cursor pointed to by this object, wrapped itself into a `Right`, or a `Left` with a list of failures
  *   in case an error occurred during navigation
  */
case class FluentConfigCursor(cursor: ConfigReader.Result[ConfigCursor]) {
  ...
  /** Casts this cursor to a `ConfigListCursor` and maps each element to a result. This method tries to map all
    ...
    * @return
    *   a `Right` with the list obtained by mapping all elements of the list pointed to by this cursor if all casts and
    *   mappings can be done, `Left` with a list of failures otherwise.
    */
  ...
  /** Casts this cursor to a `ConfigObjectCursor` and maps each value to a result. This method tries to map all
    ...
    * @return
    *   a `Right` with the map obtained by mapping all values of the object pointed to by this cursor if all casts and
    *   mappings can be done, `Left` with a list of failures otherwise.
    */
