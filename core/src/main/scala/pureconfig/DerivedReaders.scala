...
trait DerivedReaders extends DerivedReaders1 {
  ...
  implicit def deriveTupleInstance[F: IsTuple, Repr <: HList, LRepr <: HList, DefaultRepr <: HList](
    ...
    override def from(value: ConfigCursor) = {
      ...
      value.value.valueType match {
        case ConfigValueType.OBJECT => deriveTupleInstanceAsObject(value)
        case ConfigValueType.LIST => deriveTupleInstanceAsList(value)
        case other => fail(WrongType(other, Set(ConfigValueType.LIST, ConfigValueType.OBJECT), ConfigValueLocation(value.value), ""))
      }
    }
  }

  private[pureconfig] def deriveTupleInstanceAsList[F: IsTuple, Repr <: HList](value: ConfigCursor)(
    implicit
    gen: Generic.Aux[F, Repr],
    cr: ConfigReader[Repr]): Either[ConfigReaderFailures, F] =
    cr.from(value).right.map(gen.from)

  private[pureconfig] def deriveTupleInstanceAsObject[F: IsTuple, Repr <: HList, DefaultRepr <: HList](value: ConfigCursor)(
    implicit
    gen: LabelledGeneric.Aux[F, Repr],
    default: Default.AsOptions.Aux[F, DefaultRepr],
    cr: WrappedDefaultValue[F, Repr, DefaultRepr]): Either[ConfigReaderFailures, F] =
    cr.fromWithDefault(value.value, default()).right.map(gen.from)
}
...
trait DerivedReaders1 {
        ...
        (co.get(keyStr), vFieldReader.value.value) match {
          case (null, reader) =>
            default.head match {
              ...
              case _ if reader.isInstanceOf[AllowMissingKey] => reader.from(null: ConfigValue)
  ...
  implicit final def cNilConfigReader[Wrapped]: WrappedConfigReader[Wrapped, CNil] = new WrappedConfigReader[Wrapped, CNil] {
    override def from(config: ConfigCursor): Either[ConfigReaderFailures, CNil] =
      fail(NoValidCoproductChoiceFound(config.value, ConfigValueLocation(config.value), ""))
  }
  ...
  implicit final def coproductConfigReader[Wrapped, Name <: Symbol, V, T <: Coproduct](
    ...
    new WrappedConfigReader[Wrapped, FieldType[Name, V] :+: T] {
      ...
      override def from(config: ConfigCursor): Either[ConfigReaderFailures, FieldType[Name, V] :+: T] =
        coproductHint.from(config.value, vName.value.name) match {
  ...
  class OptionConfigReader[T](implicit conv: Derivation[Lazy[ConfigReader[T]]]) extends ConfigReader[Option[T]] with AllowMissingKey {
    override def from(config: ConfigCursor): Either[ConfigReaderFailures, Option[T]] = {
      if (config.value == null || config.value.unwrapped() == null)
        Right(None)
      else
  ...
  implicit def deriveTraversable[T, F[T] <: TraversableOnce[T]](
    ...
    override def from(config: ConfigCursor): Either[ConfigReaderFailures, F[T]] = {
      config.value match {
        case co: ConfigList =>
  ...
  implicit def deriveMap[T](implicit configConvert: Derivation[Lazy[ConfigReader[T]]]) = new ConfigReader[Map[String, T]] {
    ...
    override def from(config: ConfigCursor): Either[ConfigReaderFailures, Map[String, T]] = {
      config.value match {
  ...
  implicit final lazy val hNilConfigReader: ConfigReader[HNil] =
    new ConfigReader[HNil] {
      def from(cv: ConfigCursor): Either[ConfigReaderFailures, HNil] = {
        cv.value match {
          case cl: ConfigList if cl.size == 0 => Right(HNil)
          case cl: ConfigList => fail(WrongSizeList(0, cl.size, ConfigValueLocation(cv.value), ""))
  ...
  implicit final def hConsConfigReader[H, T <: HList](implicit hr: Derivation[Lazy[ConfigReader[H]]], tr: Lazy[ConfigReader[T]], tl: HKernelAux[T]): ConfigReader[H :: T] =
    new ConfigReader[H :: T] {
      def from(cv: ConfigCursor): Either[ConfigReaderFailures, H :: T] = {
        cv.value match {
          case cl: ConfigList if cl.size != tl().length + 1 => fail(WrongSizeList(tl().length + 1, cl.size, ConfigValueLocation(cv.value), ""))
          case cl: ConfigList =>
            val sl = cl.asScala
  ...
  implicit final def deriveProductInstance[F, Repr <: HList, DefaultRepr <: HList](
    ...
    override def from(config: ConfigCursor): Either[ConfigReaderFailures, F] = {
      cc.value.fromWithDefault(config.value, default()).right.map(gen.from)
    }
  }
