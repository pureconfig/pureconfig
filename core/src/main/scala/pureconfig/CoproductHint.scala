...
class FieldCoproductHint[T](key: String) extends CoproductHint[T] {
    ...
    case co: ConfigObject =>
      Option(co.get(key)) match {
        case Some(fv) => fv.unwrapped match {
          ...
          case _ => Left(ConfigReaderFailures(WrongType(s"${fv.valueType}", s"${ConfigValueType.STRING}", ConfigValueLocation(fv), Some(key))))
        }
        case None => Left(ConfigReaderFailures(KeyNotFound(key, ConfigValueLocation(co))))
      }
    case _ => Left(ConfigReaderFailures(WrongType(s"${cv.valueType}", s"${ConfigValueType.OBJECT}", ConfigValueLocation(cv), None)))
  }
  ...
  def to(cv: ConfigValue, name: String): Either[ConfigReaderFailures, ConfigValue] = cv match {
    ...
    case _ =>
      Left(ConfigReaderFailures(WrongType(s"${cv.valueType}", s"${ConfigValueType.OBJECT}", ConfigValueLocation(cv), None)))
  }
