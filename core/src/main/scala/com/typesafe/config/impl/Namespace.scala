...
/**
 * A public wrapper over the package-private [[com.typesafe.config.impl.Path]]. This exposes a way to convert Typesafe
 * Config namespaces from strings to a list of keys and vice-versa using their internal path parser.
 */
object Namespace {
  ...
  /**
   * Converts a list of keys to a path string compatible with the `get` methods in [[com.typesafe.config.Config]].
   *
   * @param path the list of keys to convert to a string
   * @return a path string compatible with the `get` methods in [[com.typesafe.config.Config]].
   */
