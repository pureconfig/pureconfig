...
class ProductHintSuite extends BaseSuite {
  ...
  it should "allow customizing the field mapping through a product hint" in {
    ...
    val default = ConfigReader.derived[SampleConf]
    val customized =
      given ProductHint[SampleConf] = ProductHint(ConfigFieldMapping(_.toUpperCase))
      ConfigReader.derived[SampleConf]
  ...
  it should "disallow unknown keys if specified through a product hint" in {
    ...
    case class Conf1(a: Int) derives ConfigReader
    case class Conf2(a: Int) derives ConfigReader
  ...
  it should "accumulate all failures if the product hint doesn't allow unknown keys" in {
    ...
    case class Conf(a: Int) derives ConfigReader
  ...
  it should "include candidate keys in failure reasons in case of a suspected misconfigured ProductHint" in {
    case class CamelCaseConf(camelCaseInt: Int, camelCaseString: String) derives ConfigReader
    case class KebabCaseConf(kebabCaseInt: Int, kebabCaseString: String) derives ConfigReader
    case class SnakeCaseConf(snakeCaseInt: Int, snakeCaseString: String) derives ConfigReader
