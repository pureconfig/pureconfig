...
class ConfigSourceSuite extends BaseSuite {
  ...
  it should "allow reading scalar values from a config object at a namespace" in {
    ...
    source.at("foo.bar.h").load[Option[Float]] should failWith(
  ...
  it should "handle correctly namespaces with special chars" in {
    ...
    source.at("\"b.a.r\".foo.f").load[Float] should failWith(KeyNotFound("b.a.r", Set.empty), "", stringConfigOrigin(1))
  }
  ...
  it should "allow chaining required fallback sources" in {
    ...
    overrides.withFallback(main).withFallback(defaults).at("my-service").load[MyService] shouldBe Right(
      MyService("example.com", 8081, true)
    )
    ...
    overrides
      ...
      .at("my-service")
      .load[MyService] should matchPattern {
    ...
    overrides
      ...
      .load[MyService]
      .left
      .map(_.toList) should matchPattern {
      case Left(List(CannotReadResource("nonExistingResource1", _), CannotReadResource("nonExistingResource2", _))) =>
    }
  }
  ...
  it should "allow chaining optional fallback sources" in {
    ...
    overrides.optional
      ...
      .at("my-service")
      .load[MyService] shouldBe Right(MyService("example.com", 8081, true))
