...
class BasicConvertersSuite extends BaseSuite {
  ...
  checkRead[FiniteDuration](ConfigValueFactory.fromAnyRef(28) -> 28.millis)
  ...
  checkFailure[DayOfWeek, CannotConvert](
    ...
    ConfigValueFactory.fromAnyRef("this is not a day")
  ) // no such value
  ...
  checkReadWrite[ConfigMemorySize](ConfigValueFactory.fromAnyRef(400L) -> ConfigMemorySize.ofBytes(400L))
...

  {
    ...
    checkReadWrite[ConfigObject](conf.root() -> ConfigValueFactory.fromMap(Map("v1" -> 3, "v2" -> 4).asJava))

    checkReadWrite[Config](conf.root() -> conf)
  }
}
