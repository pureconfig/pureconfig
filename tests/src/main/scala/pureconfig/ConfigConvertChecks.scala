...
trait ConfigConvertChecks { this: AnyFlatSpec with Matchers with ScalaCheckDrivenPropertyChecks with EitherValues =>
   ...
   */
  def checkArbitrary[A](implicit cc: Derivation[ConfigConvert[A]], arb: Arbitrary[A], tpe: TypeTag[A], equality: Equality[A]): Unit =
   ...
   */
  def checkArbitrary2[A, B](f: B => A)(implicit cr: ConfigConvert[A], cw: ConfigConvert[B], arb: Arbitrary[B], tpe1: TypeTag[A], tpe2: TypeTag[B], equality: Equality[A]): Unit =
   ...
   */
  def checkFailure[A, E <: FailureReason](values: ConfigValue*)(implicit cr: ConfigReader[A], tpe: TypeTag[A], eTag: ClassTag[E]): Unit =
    for (value <- values) {
   ...
   * `ConfigValue`.
   */
  def checkFailures[A](valuesToErrors: (ConfigValue, ConfigReaderFailures)*)(implicit cr: ConfigReader[A], tpe: TypeTag[A]): Unit =
