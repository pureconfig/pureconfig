...
class HadoopModuleTest extends BaseSuite {
  ...
  it should "be able to read correct path value from config" in {
    ...
    configString(strPath).to[Path].value shouldEqual new Path(strPath)
  }
  ...
  it should "refuse an invalid path value" in {
    configString("file:#//123").to[Path] should failWithReason[CannotConvert]
  }

  it should "refuse an empty path value" in {
    configString("").to[Path] should failWithReason[EmptyStringFound]
  }
}
