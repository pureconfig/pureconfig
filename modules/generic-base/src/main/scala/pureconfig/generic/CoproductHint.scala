...
object CoproductHint {
  ...
  /** An action to only use the provided `ConfigCursor` and not try other options.
    ...
    */
  case class Use(cursor: ConfigCursor, option: String)
      extends Action

      /** An action to attempt to use the provided coproduct options, in the specified order, stopping at the first one
