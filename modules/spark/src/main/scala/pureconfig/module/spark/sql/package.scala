...
package object sql {
  ...
  implicit val structFieldReader: ConfigReader[StructField] =
    ConfigReader.forProduct4("name", "data-type", "nullable", "metadata") {
  ...
  implicit val structFieldWriter: ConfigWriter[StructField] =
    ConfigWriter.forProduct4("name", "data-type", "nullable", "metadata") { case StructField(name, dt, n, m) =>
      (name, dt, n, m)
    }
}
